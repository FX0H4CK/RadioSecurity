...
def home():
        ...
        return render_template("index.html", percent=percent, color=color, percent_enc=percent_enc,color_enc=color_enc)
...
def wifi():
    if request.method == "POST":
        ...
        mac = accept.split(',',1)[0].replace(' ','')
        ssid = accept.split(',',1)[1].replace(' ','')
        print(mac)
        print(ssid)
        try:
            conn = mariadb.connect(
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("UPDATE ssids SET status=1 WHERE bssid='"+mac+"' AND essid=' "+ssid+"';")
            print("UPDATE ssids SET status=1 WHERE bssid='"+mac+"' AND essid=' "+ssid+"';")
            conn.commit()
            conn.close()
        except mariadb.Error as e:
        ...
        for i in range(1,len(ssids)):
                exec("""global SSID%d; SSID%d = ssids[%d][13].replace('" "', 'Versteckt').replace('"', '');""" % (i,i,i));
                exec("""global ENC%d; ENC%d = ssids[%d][5].replace('"', '');""" % (i,i,i)); 
                exec("""global MAC%d; MAC%d = ssids[%d][0].replace('"', '');""" % (i,i,i));
                exec("""global ls%d; ls%d = ssids[%d][2];""" % (i,i,i));
                exec("""global vendor%d; vendor%d = ssids[%d][14];"""% (i,i,i));
        conn.close()
...
def knownwifi():
    if request.method == "POST":
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("UPDATE ssids SET status=NULL WHERE bssid='"+mac+"' AND essid=' "+ssid+"';")
            conn.commit()
            conn.close()
        except mariadb.Error as e:
            print(f"[x] Error while executing update: {e}")
